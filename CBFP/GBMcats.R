...
for (i in pid){
  ...
  save(GBM,file =paste0("E:/targetprofiling/model/GBM","/",i,".cats.Rdata"))
  ypre = sign(GBM$cv.fitted)
  ypre[ypre==-1] = 0
  GBMcatsper[i] = sum((ypre-ydata)!=0)/length(ydata)
  print(i)
}

############################################################################

save(GBMcatsper,file ="E:/targetprofiling/model/GBMcatsper.Rdata")