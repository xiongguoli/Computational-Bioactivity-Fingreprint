...
for (i in pid){
  ...
  save(GBM,file =paste0("E:/targetprofiling/model/ADA","/",i,".cats.Rdata"))
  ypre = sign(GBM$cv.fitted)
  ypre[ypre==-1] = 0
  ADAcatsper[i] = sum((ypre-ydata)!=0)/length(ydata)
  print(i)
}

############################################################################

save(ADAcatsper,file ="E:/targetprofiling/model/ADAcatsper.Rdata")