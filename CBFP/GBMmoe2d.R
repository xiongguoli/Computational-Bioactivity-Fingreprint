...
for (i in pid){
  ...
  save(GBM,file =paste0("E:/targetprofiling/model/GBM","/",i,".moe2d.Rdata"))
  ypre = sign(GBM$cv.fitted)
  ypre[ypre==-1] = 0
  GBMmoe2dper[i] = sum((ypre-ydata)!=0)/length(ydata)
  print(i)
}

############################################################################

save(GBMmoe2dper,file ="E:/targetprofiling/model/GBMmoe2dper.Rdata")